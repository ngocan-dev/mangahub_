...
CREATE INDEX IF NOT EXISTS idx_novels_rating ON Novels(Rating_Point) WHERE Rating_Point IS NOT NULL;
...
CREATE INDEX IF NOT EXISTS idx_novels_search_composite ON Novels(Status, Genre, Rating_Point)
WHERE Status IS NOT NULL AND Genre IS NOT NULL AND Rating_Point IS NOT NULL;
