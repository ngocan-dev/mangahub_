...
func buildMangaHandler(conn *sql.DB, mangaService *manga.Service) *MangaHandler {
	...
	ratingRepo := rating.NewRepository(conn)
	...
	favoriteService := favorite.NewService(favoriteRepo, mangaService, historyService, ratingService)
	commentService := comment.NewService(commentRepo, mangaService, ratingService)
